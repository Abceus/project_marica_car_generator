...
{
    auto image = ResourceManager::Instance().getTextureManager().get( filename );
    if( !image->isNull() )
    {
        if( averageAlpha( *image.get() ) < 240.0f && !m_transparentBuffers.contains( index ) )
        {
    ...
    else
    {
        ...
        image = ResourceManager::Instance().getTextureManager().get( "./resources/textures/DefaultTexture.tga" );
    ...
    this->textures[index] = QSharedPointer<QOpenGLTexture>( new QOpenGLTexture( *image.get() ) );
}
...
//        image = loadTga( "./resources/textures/DefaultTexture.tga", success );
    ...
    this->textures.append( QSharedPointer<QOpenGLTexture>( new QOpenGLTexture( *image.get() ) ) );
}
