...
OpenglView::OpenglView() : MulOpenglCanvas() {
...
void OpenglView::onInit() {
    ...
    openglInitedCollection.call();
    //wxPostEvent(this, wxCommandEvent(OPENGL_INITED));
}

void OpenglView::onKeyPressed(const ImGuiInputEvent& event) {
...
void OpenglView::addOpenglInitedCallback(OpenglInitedCallbackType& outHandler, const std::function<void()>& callback) {
    outHandler = std::make_shared<MulCallbackOwner<>>(callback);
    openglInitedCollection.addCallback(outHandler);
}